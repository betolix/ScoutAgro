...
app.post('/', ( req, res) => {
  ...
  // console.log('body ', req.body);
    ...
    pool.query('select _id, usuario, primer_nombre, segundo_nombre, apellido_paterno, apellido_materno, tipo_documento, numero_documento, email_1, email_2, operador_celular, numero_celular, fecha_nacimiento, role_id, password, img, google from t_usuario WHERE email_1= $1 ', [body.email_1], (err, usuarioDB) => {
        ...
        var menu = ()=>{obtenerMenuX( usuarioDB.rows[0].role );} 
        console.log('Menu YYYY', menu)
        res.status(200).json({
            ok: true,
            usuario: usuarioDB.rows[0],
            token: token,
            id: usuarioDB.rows[0]._id,
            menu: 1//obtenerMenuX( usuarioDB.rows[0].role )
            // menu: ()=>{ obtenerMenuX( usuarioDB.rows[0].role );}
